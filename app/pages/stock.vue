...
const startEditCategory = () => {
  ...
  editingCategoryValue.value = detailPart.value.category
...
const saveCategory = async () => {
  if (!detailPart.value) return
  try {
    await inventree.updatePart(detailPart.value.pk, { category: editingCategoryValue.value })
    detailPart.value.category = editingCategoryValue.value
    // Also update in the main list
    const inList = parts.value.find(p => p.pk === detailPart.value!.pk)
    if (inList) inList.category = editingCategoryValue.value
...
<template>
  <div class="container mx-auto p-6 max-w-6xl">
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-2xl font-bold mb-1">View Stock</h1>
      <p class="text-sm text-gray-600 dark:text-gray-400">Browse all parts and their current stock levels</p>
    </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700">
              <th class="text-left px-4 py-3 font-semibold text-gray-700 dark:text-gray-300">Part</th>
              <th class="text-left px-4 py-3 font-semibold text-gray-700 dark:text-gray-300">IPN</th>
              <th class="text-left px-4 py-3 font-semibold text-gray-700 dark:text-gray-300">Rev</th>
              ...
              <th class="text-right px-4 py-3 font-semibold text-gray-700 dark:text-gray-300">Stock</th>
    ...
    <UModal v-model:open="isDetailOpen">
      <template #content>
        <div class="p-6 max-w-2xl w-full">
          <div v-if="detailPart" class="space-y-4">
            <!-- Part info header -->
            <div class="flex items-start justify-between">
              <div>
                <h2 class="text-lg font-bold">{{ detailPart.name }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ detailPart.description || 'No description' }}</p>
              </div>
              <UButton variant="ghost" color="neutral" icon="i-lucide-x" size="sm" @click="isDetailOpen = false" />
            </div>

            <!-- Part metadata -->
            <div class="grid grid-cols-3 gap-3 text-sm">
              <div>
                <p class="text-xs text-gray-500 dark:text-gray-400">IPN</p>
                <p class="font-mono font-medium">{{ detailPart.IPN || '—' }}</p>
              </div>
              <div>
                <p class="text-xs text-gray-500 dark:text-gray-400">Revision</p>
                <p class="font-medium">{{ detailPart.revision || '—' }}</p>
              </div>
              <div>
                <p class="text-xs text-gray-500 dark:text-gray-400">Total Stock</p>
                <p class="font-bold" :class="detailPart.in_stock > 0 ? 'text-green-600' : 'text-red-600'">
                  {{ detailPart.in_stock }}
                </p>
              </div>
            </div>
            ...
            <div class="text-sm">
              ...
              <div v-if="isEditingCategory" class="flex items-center gap-2">
                <USelectMenu
                  ...
                  class="flex-1"
                />
                <UButton size="xs" icon="i-lucide-check" color="success" variant="ghost" @click="saveCategory" />
                <UButton size="xs" icon="i-lucide-x" color="neutral" variant="ghost" @click="cancelEditCategory" />
              </div>
              <button
                v-else
              ...
              >
                ...
                <UIcon name="i-lucide-pencil" class="w-3 h-3 opacity-50" />
              </button>
            </div>
            ...
            <!-- Stock items table -->
            <div>
              <h3 class="text-sm font-semibold mb-2">Stock Items</h3>

              <div v-if="isLoadingDetail" class="text-center py-4 text-gray-500">
                <UIcon name="i-lucide-loader-2" class="w-4 h-4 animate-spin inline-block mr-1" />
                Loading stock items...
              </div>

              <div v-else-if="detailStockItems.length === 0" class="text-center py-4 text-gray-500 text-sm">
                No stock items for this part.
              </div>

              <div v-else class="overflow-x-auto rounded-md border border-gray-200 dark:border-gray-700">
                <table class="w-full text-sm">
                  <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                      <th class="text-left px-3 py-2 font-medium text-gray-600 dark:text-gray-400">Vendor (Batch)</th>
                      <th class="text-right px-3 py-2 font-medium text-gray-600 dark:text-gray-400">Quantity</th>
                      <th class="text-left px-3 py-2 font-medium text-gray-600 dark:text-gray-400">Serial</th>
                  ...
                  <tbody>
                    ...
                    >
                      <td class="px-3 py-2">
                        <!-- Editing mode -->
                        <div v-if="editingBatchPk === item.pk" class="flex items-center gap-2">
                          <USelectMenu
                            ...
                            class="w-32"
                          />
                          <UButton size="xs" icon="i-lucide-check" color="success" variant="ghost" @click="saveBatch(item)" />
                          <UButton size="xs" icon="i-lucide-x" color="neutral" variant="ghost" @click="cancelEditBatch" />
                        </div>
                        <!-- Display mode -->
                        <button
                          v-else
                        ...
                        >
                          <span v-if="item.batch" class="font-medium">{{ item.batch }}</span>
                          <span v-else class="text-gray-400 italic">No vendor — click to set</span>
                          <UIcon name="i-lucide-pencil" class="w-3 h-3 opacity-50" />
                        </button>
                      </td>
      ...
      <template #content>
        <div class="p-6 max-w-md w-full">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-base font-semibold">Select Stock Item to Print</h3>
            <UButton variant="ghost" color="neutral" icon="i-lucide-x" size="sm" @click="isVendorPickerOpen = false" />
          </div>
          ...
          <div class="space-y-2">
            ...
            >
              <div>
                <p class="text-sm font-medium">{{ item.batch || 'No vendor' }}</p>
